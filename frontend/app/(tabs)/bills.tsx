...
export default function BillsScreen() {
  ...
  useEffect(() => {
    fetchCurrentWeekBills();
  }, []);

  const fetchCurrentWeekBills = async () => {
    setLoading(true);
    try {
      const response = await axios.get(`${API_URL}/api/bills/current-week`);
  ...
  if (!weekData || weekData.bills.length === 0) {
    return (
      <View style={styles.centerContainer}>
        <Ionicons name="document-text-outline" size={64} color="#bdbdbd" />
        <Text style={styles.emptyText}>No entries for current week</Text>
          ...
          <View style={styles.billContainer}>
            <View style={styles.billHeader}>
              ...
              <Text style={styles.billDate}>
                {formatDate(weekData.week_start)} to {formatDate(weekData.week_end)}
              </Text>
            </View>
    ...
    <ScrollView style={styles.container}>
      <View style={styles.weekInfo}>
        ...
        <Text style={styles.weekText}>
          Current Week: {formatDate(weekData.week_start)} to{' '}
          {formatDate(weekData.week_end)}
        </Text>
      </View>

      {weekData.bills.map((bill, index) => (
        <TouchableOpacity
          key={bill.customer_id || index}
          style={styles.billCard}
          onPress={() => setSelectedBill(bill)}
        >
          <View style={styles.billCardHeader}>
            <Text style={styles.billCardName}>{bill.customer_name}</Text>
            <Ionicons name="chevron-forward" size={24} color="#757575" />
          </View>
          <View style={styles.billCardDetails}>
            <View style={styles.billCardRow}>
              <Text style={styles.billCardLabel}>Entries:</Text>
              <Text style={styles.billCardValue}>{bill.entries.length}</Text>
            </View>
            <View style={styles.billCardRow}>
            ...
            <View style={styles.billCardRow}>
              ...
              <Text
                style={[
                  ...
                  bill.balance > 0 ? styles.balanceDue : styles.balancePaid,
                ]}
              >
